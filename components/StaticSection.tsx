  ...
  return (
    <>
      ...
      <div className="hidden lg:block mt-8">
        {/* Nodes + absolute connector line */}
        <div className="relative flex items-start">
          {/* Dotted line behind nodes — positioned at circle center */}
          <div
            aria-hidden
          ...
          {nodes.map(node => (
            ...
            >
              {/* Integration label — fixed height so connector aligns */}
              <div
      ...
      <div className="lg:hidden flex flex-col mt-6">
        {nodes.map((node, i) => (
          <div key={node.num} className="flex gap-3">
            {/* Left: circle + vertical dotted line */}
            <div className="flex flex-col items-center flex-shrink-0">
              ...
              {i < nodes.length - 1 && (
                <div
                  ...
                  style={{
                    width:       '2px',
                    borderLeft:  `2px dashed ${accent}4D`,
                    minHeight:   '28px',
                  }}
                />
              )}
...
/* ─── Expanded project card (with pipeline) ───────────────── */
...
function ProjectsSection() {
  const standard = ALL_PROJECTS.filter(p => !p.pipeline)
  const expanded  = ALL_PROJECTS.filter(p => p.pipeline)
  ...
  return (
    ...
    >
      <div className="max-w-6xl mx-auto">
        ...
        {/* Expanded full-width cards (with pipeline) */}
        {expanded.length > 0 && (
                ...
                {tier.items.map((skill, i) => (
                  <Reveal key={skill.name} delay={i * 60}>
                    ...
                    >
                      ...
                      <p className="text-[11px] leading-relaxed mb-2" style={{ color: 'var(--color-muted)' }}>
                        {skill.desc}
                      </p>
